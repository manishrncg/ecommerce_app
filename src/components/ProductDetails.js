...
class ProductDetails extends React.Component {
	...
	render() {  // Basic component for every product being displayed while listing
		const {
	  	...
	  	const discount = parseFloat(this.primary_Details.mark_price) - parseFloat(this.primary_Details.sale_price);
	  	const discountPercent = discount/100;
		let optionsVar = this.renderAttributes(this.options, this.attributes, attributesChecked, selected_option_ids);

	  	if(this.primary_Details.name){
	  		const productImages = this.renderProductImages(this.primary_Details.name, this.primary_Details.images);
				...
				<React.Fragment>
					...
					<div className="col-md-8 col-sm-8 col-xs-8  offset-md-2 offset-sm-2 offset-xs-2 productMarginTop" >
						<div className="row">
							...
							<div className="col-md-6 col-sm-6 col-xs-6" >
								<h4>{this.primary_Details.name}</h4>
								<p className={show}>{this.primary_Details.desc}</p>
								<p className={hideMore} onClick={e => this.revealText(e)}>+MORE</p>
								<hr/>
								<div>&#8377; {this.primary_Details.sale_price.toFixed(2)} 
									<span className="mark_price">
										&#8377; {this.primary_Details.mark_price.toFixed(2)}
									</span>
								</div>
